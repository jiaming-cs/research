...
double draw_match_64(Mat img1, Mat img2, Mat affineMat, int angle, double scale, int matchNum = 50, bool extended = false){
    ...
    //copy(matches.begin(), matches.begin() + min(matchNum, (int)matches.size()), good_matches.begin());
    good_matches.assign(matches.begin(), matches.begin() + min(matchNum, (int)matches.size()));
...
double draw_match(Mat img1, Mat img2, Mat affineMat, int angle, double scale, int matchNum = 50, bool extended = true){
...
int main(int argc, char const *argv[])
{
    ...
    vector<double> vc_acc_64;
    vector<double> vc_acc_normal;
    vector<double> mean_acc_64;
...
for (int i = 0 ; i < 11; i++){
    scale = 0.5 + i * 0.1;
        for (int angle=0; angle<=360; angle += step){
            ...
            double acc_64 = draw_match_64(pad, rot, affineMat, angle, scale, matchNum);
            double acc_normal = draw_match(pad, rot, affineMat, angle, scale, matchNum);
            cout<<"64: "<<acc_64<<endl;
            cout<<"128: "<<acc_normal<<endl;
            vc_acc_64.push_back(acc_64);
            vc_acc_normal.push_back(acc_normal);
    }
    double mean = accumulate(vc_acc_64.begin(), vc_acc_64.end(), 0.0) / vc_acc_64.size();
    mean_acc_64.push_back(mean);
    ...
    for (int i = 0; i<mean_acc_64.size(); i++){
        cout<<mean_acc_64[i]<<", ";
    ...
    imshow("match_64", matches_img_64);
    imwrite("matches.png", matches_img);
    imwrite("64_matches.png", matches_img_64);
    waitKey(100000);
    */
    return 0;
}
