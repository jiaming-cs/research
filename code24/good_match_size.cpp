...
void draw_match_double_good_match_num(Mat& img1,
                Mat& img2,
                Mat& img_matches,
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
	{
        ...
        draw_match_double_good_match_num(frame1, frame2, out1, detector_a, detector_b);
        if (!out1.empty())
            imshow("Good Match Number = 100", out1);

        draw_match_double_surf(frame1, frame2, out2, detector1, detector2);
        if (!out2.empty())
