...
void draw_match_with_increase(Mat& img1,
                Mat& img2,
                Mat& img_matches,
...
void draw_match_only_decrease(Mat& img1,
                Mat& img2,
                Mat& img_matches,
                Ptr<SurfFeatureDetector> &detector1,
                Ptr<SurfFeatureDetector> &detector2,
    ...
    while ((key_points_img2.size() < min_size || key_points_img2.size() > max_size)){
        if ( key_points_img2.size() < min_size ){
            if (hessian <= min_hessian)
                break;
            hessian = (int)hessian/1.5;
...
int main(int argc, char const *argv[])
{
	...
	cp2.open("http://10.10.10.108:4747/video");
    Mat frame1, frame2;
    Mat out1, out2;

    Ptr<SurfFeatureDetector> detector_a = new SurfFeatureDetector(800);
    Ptr<SurfFeatureDetector> detector_b = new SurfFeatureDetector(800);
    Ptr<SurfFeatureDetector> detector1 = new SurfFeatureDetector(800);
    Ptr<SurfFeatureDetector> detector2 = new SurfFeatureDetector(800);
    ...
    while (1)
	{
        ...
        draw_match_with_increase(frame1, frame2, out1, detector_a, detector_b);
        if (!out1.empty())
            imshow("With Increase", out1);

        draw_match_only_decrease(frame1, frame2, out2, detector1, detector2);
        if (!out2.empty())
            imshow("Only Decrease", out2);
    ...
    draw_match_double_good_match_num(img1, img2, out1, detector_a, detector_b);
    if (!out1.empty())
        imshow("Good Match Number = 100", out1);

    draw_match_double_surf(img1, img2, out2, detector1, detector2);
    if (!out2.empty())
        imshow("Ratio = 0.8", out2);
