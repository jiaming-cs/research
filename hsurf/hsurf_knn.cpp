...
Mat draw_match_hsv(Mat img1, Mat img2, Mat affineMat, double weight, int matchNum = 50, double p = 0.05, bool extended = false){
    ...
    BFMatcher matcher;
    vector<vector<DMatch>>matches; // candidate matches

    //int k = max(2, (int)(min(key_points_img1.size(), key_points_img2.size())*p)); // minimum is 2, maximum is p of the number of keypoint 
    int k = 5;
    matcher.knnMatch(descriptor_img1, descriptor_img2, matches, k);
    ...
    for(int i = 0; i < matches.size(); i++)
    {
        ...
        if (matches[i][0].distance < ratio * matches[i][1].distance)
        {
            ...
            for (int j=0; j<k; j++){
                int query_index = matches[i][j].queryIdx;
                int train_index = matches[i][j].trainIdx;
                ...
                double t = matches[i][j].distance;
                matches[i][j].distance = (1 - weight) * t + weight * (1-hsv_distance) * t;
            }
